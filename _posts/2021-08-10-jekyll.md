...
Jekyll은 동적 객체 지향 스크립트 언어인 루비(*Ruby*) 기반으로 개발된 하나의 라이브러리 즉 루비젬(*RubyGem*)이다.<!--more-->
루비 언어의 특성에 따라 속도가 느리지만 매번 html 태그를 작성하지 않고 콘텐츠에 초점을 맞춘 Markup 언어로 작성하여 보다 쉽게 정적 웹사이트를 생성하는 편리한 도구이다.
...
초안이란 파일명에 날짜가 없는 포스트이다. 현재 작성중으로 아직은 게시하고 싶지 않은 포스트를 의미한다. 초안 기능을 사용하려면 `_drafts` 디렉토리 내 초안을 작성한다.